...
(function() {
  ...
  const ACTIONS = [
    ...
    {
      type: "restart_scene",
      label: "🔄 Redémarrer la scène",
      advLabel: "restart_scene",
      desc: "Recharge la scène actuelle depuis son état de départ : objets, spawn joueur, collisions et timers.",
      params: []
    },
    {
      type: "player_move",
  ...
  function runActions(actions, sim) {
    ...
    for (let i = 0; i < actions.length; i++) {
      ...
      try {
        ...
        } else if (a.type === "restart_scene") {
          const currentSceneId = sim && sim.scene ? sim.scene.id : null;
          if (currentSceneId == null) {
            console.warn("[Event Sheet] restart_scene impossible : aucune scène active");
            continue;
          }
          _runtime.pendingSceneSwitch = { sceneId: currentSceneId, restart: true };
          return { wait: false, sceneSwitch: true, restart: true };
